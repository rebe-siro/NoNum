...
template <class Tipo>
void Lista<Tipo>::Eliminar(Apuntador p) {
    if (!Primero) return; // La lista est? vac?a

    if (p == Primero) { // Si el nodo a eliminar es el primero
        Primero = p->prox;
        delete p;
        return;
    }

    Apuntador anterior = Primero;
    while (anterior->prox != p && anterior->prox != nullptr) {
        anterior = anterior->prox;
    }

    if (anterior->prox == nullptr) return; // El nodo a eliminar no est? en la lista
